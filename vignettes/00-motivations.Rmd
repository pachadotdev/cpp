...
# Add special case
cases <- rbind(list(len = 3e6, vector = "normal", method = "for2", pkg = "cpp4r"), cases)
...
library(cpp4rtest)
grid <- expand.grid(len = c(10^(2:5), 2e5), pkg = c("cpp4r", "Rcpp"), stringsAsFactors = FALSE)
...
ggplot(b_release, aes(x = len, y = min / len, color = pkg)) +
  ...
  labs(
    title = "Number of protected objects\nvs\nAvg. time to release protection on one object",
    x = "Number of protected objects",
    y = "Avg. time to release protection on one object"
  ) +
...
```{r, message = FALSE, eval = should_run_benchmarks()}
grid <- expand.grid(len = 10 ^ (0:7), pkg = "cpp4r", stringsAsFactors = FALSE)
grid <- rbind(
  grid,
