...
```cpp
[[cpp4r::register]] double sum_cpp_(doubles x) {
  int n = x.size();
  double total = 0;
  for(int i = 0; i < n; ++i) {
    total += x[i];
...
```cpp
[[cpp4r::register]] double mean_cpp_(doubles x) {
  int n = x.size();
  double y = 0;
...
```cpp
[[cpp4r::register]] double var_cpp_(doubles x) {
  int n = x.size();
  double y1 = 0, y2 = 0;
...
```cpp
[[cpp4r::register]] double rmse_cpp_(doubles x, double x0) {
  int n = x.size();
  double y = 0;
  for (int i = 0; i < n; ++i) {
    y += pow(x[i] - x0, 2.0);
