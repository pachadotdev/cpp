...
vignette: >
  %\VignetteIndexEntry{Statistical Functions with Missing Values}
...
The previous functions ignore `NA` values, which is not ideal for real-world data. This vignette introduces an additional argument and checks to the functions that allow the user to remove missing values (including `NaN`) from the vector. These examples were adapted from @vaughan.
...
```cpp
[[cpp4r::register]] double sum2_cpp_(doubles x, bool na_rm = false) {
  int n = x.size();
  double total = 0;
  for (int i = 0; i < n; ++i) {
    if (na_rm && ISNAN(x[i])) {
...
Unlike the sum function from vignette 3, this function has an additional argument `na_rm` that allows the user to remove missing values (including `NaN`) from the vector.
...
```cpp
[[cpp4r::register]] double mean2_cpp_(doubles x, bool na_rm = false) {
  int n = x.size();
  int m = 0;
...
Unlike the mean function from vignette 3, this function has an additional argument `na_rm` that allows the user to remove missing values (including `NaN`) from the vector.
...
```cpp
[[cpp4r::register]] double var2_cpp_(doubles x, bool na_rm = false) {
  int n = x.size();
  int m = 0;
...
Unlike the variance function from vignette 3, this function has an additional argument `na_rm` that allows the user to remove missing values (including `NaN`) from the vector.
...
```cpp
[[cpp4r::register]] double rmse2_cpp_(doubles x, double x0, bool na_rm = false) {
  int n = x.size();
  int m = 0;
  double total = 0;
...
Unlike the RMSE function from vignette 3, this function has an additional argument `na_rm` that allows the user to remove missing values (including `NaN`) from the vector.
