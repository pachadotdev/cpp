...
cpp4r makes it very simple to connect C++ to R.
...
-   Section [cpp_source](#cpp-source) shows you how to use `cpp4r::cpp_source()` to load a C++ file from disk in the same way you use `source()` to load a file of R code.

-   Section [classes](#classes) discusses how to modify attributes from cpp4r, and mentions some of the other important classes.
...
-   Section [case-studies](#case-studies) shows two real case studies where cpp4r was used to get considerable performance improvements.

-   Section [package](#package) teaches you how to add C++ code to an R package.

-   Section [more](#more) concludes the vignette with pointers to more resources to help you learn cpp4r and C++.

### Prerequisites

We'll use [cpp4r](https://github.com/r-lib/cpp4r) to call C++ from R:
...
There's a lot going on underneath the hood but cpp4r takes care of all the details so you don't need to worry about them.
...
-   We create a new numeric vector of length `n` with a constructor: `cpp4r::writable::doubles out(n)`.
...
For example, we could write a basic cpp4r version of `findInterval()` that takes two arguments, a vector of values and a vector of breaks, and locates the bin that each x falls into.
...
If you don't use `devtools::load_all()`, you'll also need to run `cpp4r::cpp_register()` before building the package.
