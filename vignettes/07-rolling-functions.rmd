...
vignette: >
  %\VignetteIndexEntry{Rolling Functions}
...
```cpp
[[cpp4r::register]] doubles cumsum_cpp_(doubles x) {
  int n = x.size();
...
```cpp
[[cpp4r::register]] doubles cumprod_cpp_(doubles x) {
  int n = x.size();
  writable::doubles out(n);
...
```cpp
[[cpp4r::register]] doubles range_cpp_(doubles x) {
  int n = x.size();
