...
vignette: >
  %\VignetteIndexEntry{Logical Functions}
...
```cpp
[[cpp4r::register]] bool any_cpp_(logicals x) {
  int n = x.size();
...
To document the C++ function, you can add the following wrapper to the R code:
...
```cpp
[[cpp4r::register]] integers which_cpp_(logicals x) {
  int n = x.size();
...
```cpp
[[cpp4r::register]] bool all_cpp_1_(logicals x) {
  int n = x.size();
  for (int i = 0; i < n; ++i) {
    if (!x[i]) {
      return false;
...
```cpp
[[cpp4r::register]] bool all_cpp_2_(logicals x) {
...
[[cpp4r::register]] bool all_cpp_3_(logicals x) {
  for (bool i : x) {
    if (!i) {
      return false;
    }
...
[[cpp4r::register]] bool all_cpp_4_(logicals x) {
