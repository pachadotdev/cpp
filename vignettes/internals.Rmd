---
title: "cpp4r internals"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{cpp4r internals}
...
The development repository for cpp4r is <https://github.com/r-lib/cpp4r>.
...
Generally when developing the C++ headers I run R with its working directory in the `cpp4rtest` directory and use `devtools::test()` to run the cpp4rtests.
...
``` r
# Assuming your working directory is `cpp4rtest/`
devtools::clean_dll()
devtools::load_all()
```
...
cpp4r is a header only library, so all source code exposed to users lives in [inst/include](https://github.com/r-lib/cpp4r/tree/main/inst/include).
R code used to register functions and for `cpp4r::cpp_source()` is in [R/](https://github.com/r-lib/cpp4r/tree/main/R).
Tests for *only* the code in `R/` is in [tests/testthat/](https://github.com/r-lib/cpp4r/tree/main/tests/testthat).
The rest of the code is in a separate [cpp4rtest/](https://github.com/r-lib/cpp4r/tree/main/cpp4rtest) package included in the source tree.
Inside [cpp4rtest/src](https://github.com/r-lib/cpp4r/tree/main/cpp4rtest/src) the files that start with `test-` are C++ tests using the [Catch](https://testthat.r-lib.org/reference/use_catch.html) support in testthat.
In addition there are some regular R tests in [cpp4rtest/tests/testthat/](https://github.com/r-lib/cpp4r/tree/main/cpp4rtest/tests/testthat).
...
All of the basic r_vector classes are class templates, the base template is defined in [cpp4r/r_vector.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/r_vector.hpp).
...
Specializations for the various types are in separate files, e.g. [cpp4r/doubles.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/doubles.hpp), [cpp4r/integers.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/integers.hpp)
...
The various methods for both functions are defined in [cpp4r/as.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/as.hpp)
...
A previous version of cpp4r used a global protection list stored in an R global option, but this caused [multiple issues](https://github.com/r-lib/cpp4r/issues/330).

These functions are defined in [protect.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/protect.hpp).
...
This exception is caught by the try/catch block defined in the `BEGIN_cpp4r` macro in [cpp4r/declarations.hpp](https://github.com/r-lib/cpp4r/blob/main/inst/include/cpp4r/declarations.hpp).
...
We require R \>=3.5 to use cpp4r, but when it was created we wanted to support back to R 3.3, but `R_ContinueUnwind()` wasn't available until R 3.5.
