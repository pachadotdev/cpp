...
If the code is already written and you do not have a very compelling need to use cpp4r you should continue to use Rcpp.
However if you *do* feel like your project will benefit from using cpp4r this vignette will provide some guidance for doing the conversion.

## Getting started

1.  Add cpp4r by calling `usethis::use_cpp4r()`.
...
2.  Start converting function by function.
    ...
    1.  Convert `#include <Rcpp.h>` to `#include <cpp4r.hpp>`.
    2.  Convert all instances of `// [[Rcpp::export]]` to `[[cpp4r::register]]`.
    3.  Grep for `Rcpp::` and replace with the equivalent cpp4r function using the cheatsheets below.
...
| Rcpp            | cpp4r (read-only)   | cpp4r (writable)              |
|-----------------|---------------------|-------------------------------|
...
Note that each cpp4r vector class has a read-only and writeable version.
The default classes, e.g. `cpp4r::doubles` are *read-only* classes that do not permit modification.
If you want to modify the data or create a new vector, use the writeable variant.

Another major difference in Rcpp and cpp4r is how vectors are grown.
...
See <https://cpp4r.r-lib.org/articles/motivations.html#growing-vectors> for more details.
...
cpp4r throws an error in these cases.
...
Note that `cpp4r::stop()` and `cpp4r::warning()` are thin wrappers around `Rf_stop()` and `Rf_warning()`.
...
// cpp4r -----------------------------------------------
using namespace cpp4r::literals; // so we can use ""_nm syntax

auto as_tibble = cpp4r::package("tibble")["as_tibble"];
...
cpp4r does *not* do this, so you must include the calls to `GetRNGstate()` and `PutRNGstate()` *yourself* if you use any of those functions in your C++ code.
...
Rcpp.h includes a number of STL headers automatically, notably `<string>` and `<vector>`, however the cpp4r headers generally do not.
...
```
 ...
 In file included from path/cpp4r/include/cpp4r.hpp:3:
 path/cpp4r/include/cpp4r/R.hpp:12:9: warning: 'STRICT_R_HEADERS' macro redefined [-Wmacro-redefined]
...
cpp4r conflicts with macros declared by some R headers unless the macros `R_NO_REMAP` and `STRICT_R_HEADERS` are defined.
If you include `cpp4r.hpp` (or, at a minimum, `cpp4r/R.hpp`) before any R headers these macros will be defined appropriately, otherwise you may see errors like

> R headers were included before cpp4r headers and at least one of R_NO_REMAP or STRICT_R_HEADERS was not defined.
...
If you use typedefs for cpp4r types or define custom types you will need to define them in a `pkgname_types.hpp` file so that `cpp_register()` can include it in the generated code.
...
``` cpp
// bad
cpp4r::writable::logicals({FALSE});

// good
cpp4r::writable::logicals({r_bool(FALSE)});

// good
cpp4r::writable::logicals({FALSE, NA_LOGICAL});
```
