...
Using `cpp4r::writable::doubles` first *copies* the input vector, so when we do the multiplication we do not modify the original data.
...
cpp4r provides two mechanisms to make interfacing with Rs C API and C++ code safer.
`cpp4r::unwind_protect()` takes a functional object (a C++11 lamdba function or `std::function`) and converts any C long jumps encountered to C++ exceptions.
...
cpp4r uses these mechanisms extensively internally when calling the R C API, which make cpp4r much safer against resource leaks than using Rcpp or calling Rs C API by hand.
...
cpp4r uses a number of these including
...
I think the majority of packages should use `LinkingTo: cpp4r` and *not* vendor the cpp4r dependency.
