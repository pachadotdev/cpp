...
In fact the original version of S written in the late 1970s was mainly a wrapper around FORTRAN routines
[(History-of-S)](https://www.r-project.org/conferences/useR-2006/Slides/Chambers.pdf).

Sometimes R code just is not fast enough. After checking that your code does not unnecesarily rewrite objects and it
is vectorised, there can still be bottlenecks.

cpp4r makes it simpler to connect C++ to R. While it is possible to write C or FORTRAN code for use in R, it will be
harder by comparison as it requires more setup time.

cpp4r provides a clean, approachable API that lets you write high-performance code, insulated from R's more complex C
API.

Typical bottlenecks that C++ can address include [@wickham2]:
...
-   Problems that require advanced data structures and algorithms that R does not provide.
    Through the standard template library (STL), C++ has efficient implementations of many important data structures,
...
[Rcpp](https://cran.r-project.org/package=Rcpp) was first published to CRAN in 2008, and
[Rcpp11](https://cran.r-project.org/package=Rcpp11) in 2014. Of these `Rcpp` has by far the widest adoption, with over
2,000 reverse dependencies as of 2020.

Rcpp has been a widely successful project, however over the years a number of issues and additional C++ features have
arisen. Adding these features to Rcpp would require a great deal of work, or in some cases would be impossible without
severely breaking backwards compatibility.

cpp4r is a fork of cpp11, a ground up rewrite of C++ bindings to R with different design trade-offs and features.
...
-   Applying [C++11 features](#c11-features).
...
Consider that you have two variables `x` and `y` that both point to the same underlying data.
...
You can illustrate this by creating a Rcpp function that multiples its input vector by 2.
...
However if you now call the `times_two_rcpp()` function you get the right output value, but now `x` is *also changed*.
...
cpp4r strives to make its functions behave similarly to normal R functions, while preserving the speed of Rcpp when read only access is needed.
Each of the r_vector classes in cpp4r has a normal *read only* version that uses copy-by-reference, and a *writable* version which uses copy-by-value.

```cpp
#include "cpp4r/doubles.hpp"

[[cpp4r::register]]
cpp4r::doubles times_two_cpp4r(cpp4r::writable::doubles x) {
  for (int i = 0; i < x.size(); ++i) {
    x[i] = x[i] * 2;
  }
  return x;
...
Using `cpp4r::writable::doubles` first *copies* the input vector, so when you do the multiplication you do not modify the original data.
...
For example, the following unsafe code would leak the memory allocated in the C++ `std::vector` `x` when the R API function `Rf_allocVector()` fails (since you cannot create a vector of `-1` size).
...
cpp4r provides two mechanisms to make interfacing with R's C API and C++ code safer.
`cpp4r::unwind_protect()` takes a functional object (a C++ lamdba function or `std::function`) and converts any C long jumps encountered to C++ exceptions.
...
`cpp4r::safe()` is a more concise way to wrap a particular R API function with `unwind_protect()`.
...
Again using `cpp4r::safe()` converts the C long jump to a C++ exception, so the memory is automatically released.

cpp4r uses these mechanisms extensively internally when calling the R C API, which make cpp4r much safer against resource leaks than using Rcpp or calling R's C API by hand.

### C++ safety

In the inverse of C safety you also need to ensure that C++ exceptions do not reach the C call stack, as they will terminate R if that occurs.
Like Rcpp, cpp4r automatically generates `try / catch` guards around registered functions to prevent this and also converts C++ exceptions into normal R errors.
This is done without developer facing code changes.

With both C and C++ sides of the coin covered you can safely use R's C API and C++ code together with C++ objects without leaking resources.
...
Whereas cpp4r objects preserve the ALTREP object.

```cpp
#include "cpp4r/integers.hpp"

[[cpp4r::register]]
cpp4r::integers identity_cpp4r(cpp4r::integers x) {
  return x;
}
```

```r
y <- identity_cpp4r(1:100000)
lobstr::obj_size(y)
```
...
cpp4r retains them as ALTREP vectors, so no additional memory is needed.
...
The for2 case shows an optimization you can use if you know at compile-time that you will not be dealing with ALTREP vectors.
...
```{r, message = FALSE, results = 'asis', eval = isTRUE(Sys.getenv("CPP4R_RUN_BENCHMARKS") == "true")}
library(cpp4rtest)
...
d_sum <- readRDS("sum.Rds")
knitr::kable(d_sum)
```

[cpp4rtest/src/sum.cpp](https://github.com/pachadotdev/cpp4r/blob/main/cpp4rtest/src/sum.h) contains the code ran in these benchmarks.
...
To combat this complexity cpp4r uses the [UTF-8 everywhere](http://utf8everywhere.org/) philosophy.
This means that whenever text data is converted from R data structures to C++ data structures by cpp4r the data is translated into UTF-8.
...
Concretely cpp4r always uses `Rf_translateCharUTF8()` when obtaining `const char*` from `CHRSXP` objects and uses `Rf_mkCharCE(, CE_UTF8)` when creating new `CHRSXP` objects from `const char*` inputs.

<!--TODO: unicode examples?-->

## C++11 features {#c11-features}

C++11 introduced features to the C++ language. cpp4r uses a number of these including
...
While these are useful features, many R packages do not use these advanced features.
In addition the code needed to support these features is complex and can be challenging to maintain.

cpp4r takes a more limited scope, providing only the set of r_vector wrappers for R vector types, coercion methods to and from C++ and the limited attributes necessary to support use in R packages.
...
# count lines for cpp4r headers
git clone https://github.com/pachadotdev/cpp4r
cd cpp4r
cloc inst/include/cpp4r
```
...
In contrast the headers in cpp4r contain only 3,522 lines of code in 19 files.

This reduction in complexity should make cpp4r an easier project to maintain and ensure correctness, particularly around interactions with the R garbage collector.
...
The headers in cpp4r are more easily decoupled, so you only can include only the particular headers you actually use in a source file.
This can significantly improve the compilation speed and memory usage to compile your package.

Here are some real examples of the reduction in compile time and peak memory usage after converting packages to cpp11, which has the same header structure as cpp4r.
...
Because cpp4r consists of exclusively headers this issue does not occur.
...
Because cpp4r is fully [header only](#header-only) you can vendor the code in the same way.
`cpp4r::vendor_cpp4r()` is provided to do this if you choose.

Vendoring has advantages and drawbacks however.
The advantage is that changes to the cpp4r project could never break your existing code.
The drawbacks are both minor, your package size is now slightly larger, and major, you no longer get bugfixes and new features until you explicitly update cpp4r.

The majority of packages should use `LinkingTo: cpp4r` and *not* vendor the cpp4r dependency.
However, vendoring can be appropriate for certain situations.

## Protection {#protection}

cpp4r uses a custom double linked list data structure to track objects it is managing.
...
```{r, message = FALSE, eval = isTRUE(Sys.getenv("CPP4R_RUN_BENCHMARKS") == "true")}
library(cpp4rtest)
grid <- expand.grid(len = seq(10000, 50000, by = 10000), pkg = c("cpp4r", "Rcpp"), stringsAsFactors = FALSE)
...
```{r, echo = FALSE, dev = "svg", fig.ext = "svg", eval = capabilities("cairo"), fig.width = 8, fig.height = 5}
b_release <- readRDS("release.Rds")
library(ggplot2)
ggplot(b_release, aes(x = len, y = median, color = pkg)) +
  geom_point() +
  geom_line() +
  bench::scale_y_bench_time(base = NULL) +
  scale_x_continuous(labels = scales::comma) +
  labs(
    title = "Number of protected objects\nvs\nMedian time to release protection on one object",
  ...
  tintin::scale_colour_tintin_d(option = "tintin in the land of the soviets")
```

This plot shows the average time to protect and release a given object is essentially constant for cpp4r.
...
One major difference in Rcpp and cpp4r is how vectors are grown.
...
In contrast `cpp4r` vectors grow efficiently, reserving extra space.
Because of this you can do \~10,000,000 vector appends with cpp4r in approximately the same amount of time that Rcpp does 10,000, as this benchmark demonstrates.

```{r, message = FALSE, eval = isTRUE(Sys.getenv("CPP4R_RUN_BENCHMARKS") == "true")}
grid <- expand.grid(len = seq(10000, 50000, by = 10000), pkg = "cpp4r", stringsAsFactors = FALSE)
grid <- rbind(
  grid,
  expand.grid(len = seq(10000, 50000, by = 10000), pkg = "Rcpp", stringsAsFactors = FALSE)
)
b_grow <- bench::press(.grid = grid, {
  fun <- match.fun(sprintf("%sgrow_", ifelse(pkg == "cpp4r", "", paste0(pkg, "_"))))
  bench::mark(
    fun(len),
    min_iterations = 100
  )
...
Rcpp has been and will continue to be widely successful. cpp4r is a alternative implementation of C++ bindings to R
that chooses different design trade-offs and features.
